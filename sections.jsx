...
function Header({ go, active }) {
  return (
    <header className="sticky top-0 z-30 bg-ivory/92 backdrop-blur-md border-b border-line">
      <div className="h-[76px] px-14 flex items-center justify-between">
        <button onClick={() => go('top')} className="flex items-center gap-4 active:opacity-70 transition-opacity shrink-0">
          <Logo h={34} />
          <span className="h-6 w-px bg-line"></span>
          <span className="font-gothic text-ink/60 whitespace-nowrap" style={{ fontSize: 10.5, letterSpacing: '.2em' }}>{VENUE_NAME}</span>
        </button>
